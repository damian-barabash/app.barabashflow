...
const Auth = {
  ...
  async logout() {
    ...
    ['bf_token','bf_uid','bf_role','bf_name','bf_email'].forEach(k=>localStorage.removeItem(k));
...
/* ── HTTP HELPERS ──────────────────────────── */
function hdr(extra={}) {
  return { apikey:ANON, Authorization:'Bearer '+Auth.token, 'Content-Type':'application/json', ...extra };
}
async function dbGet(table, qs='') {
  const r = await fetch(`${SB}/rest/v1/${table}${qs}`, { headers: hdr({Accept:'application/json'}) });
  const d = await r.json();
  return Array.isArray(d) ? d : (d.error ? [] : [d]);
}
...
function statusClass(s){ return {active:'active',completed:'completed',paused:'paused',archived:'archived',open:'open'}[s]||'open'; }
