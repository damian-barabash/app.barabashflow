...
const Auth = {
  ...
  isValid() {
    return !!(this.token && this.role && this.exp && Date.now() < this.exp);
  },

  requireAdmin() {
    if (!this.isValid() || this.role !== 'admin') { location.href='login.html'; return false; }
    return true;
  },
  requireClient() {
    if (!this.isValid() || this.role !== 'client') { location.href='login.html'; return false; }
    return true;
  },

  // Refresh session silently — re-login with stored credentials not possible
  // Instead: try to get user from Supabase, if 401 → logout
  async checkSession() {
    if (!this.token) return false;
    // Check expiry (15 days stored in bf_exp)
    if (this.exp && Date.now() > this.exp) {
      this.clear();
      return false;
    }
    return true;
...
function showPopup({title, text, icon, onAction, actionLabel}) {
  ...
  const iconHtml = icon
    ? `<div class="notif-popup-icon"><img src="${esc(icon)}" onerror="this.parentElement.innerHTML='🔔'" alt=""></div>`
    : `<div class="notif-popup-icon">🔔</div>`;
  item.innerHTML = `${iconHtml}
    <div class="notif-popup-body">
      <div class="notif-popup-sender">${esc(title)}</div>
      <div class="notif-popup-text">${esc(cleanText)}</div>
      ${onAction?`<span class="notif-popup-btn">${esc(actionLabel||'Przejdź →')}</span>`:''}
    </div>
  ...
  setTimeout(() => { item.classList.add('removing'); setTimeout(()=>item.remove(),300); }, 6000);
}
...
/* ── READ RECEIPTS RENDERING ── */
function readReceiptHtml(msgId, reads, senderId) {
  // Only show for sender's own messages
  if (!reads) return '';
  const isRead = reads.some(r => r.message_id === msgId);
  return `<span class="msg-reads" title="${isRead?'Przeczytano':'Wysłano'}">${isRead?'✓✓':'✓'}</span>`;
}
...
function onFileSelected(inputId, previewId, areaId, storeKey) {
  ...
  if (area) area.style.borderColor='var(--a)';
}